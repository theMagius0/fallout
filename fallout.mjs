...
Hooks.on('renderChatMessage', (message, html, data) => {
  ...
  if (addBtn.length > 0) {
    ...
    addBtn.click((ev) => {
      let falloutRoll = message.flags.falloutroll
      let actorID = message.flags.actor
...
Hooks.on('preCreateItem', (_item) => {
  if (_item.parent) {
    ...
    if (
      _item.parent.type != 'npc' &&
      _item.parent.type != 'creature' &&
      _item.type == 'special_ability'
    ) {
      ui.notifications.warn(
        `ONLY NPCs AND CREATURES CAN HAVE SPECIAL ABILITIES`,
      )
      return false
    }
    if (
      _item.parent.type != 'character' &&
      _item.parent.type != 'robot' &&
      _item.type == 'perk'
    ) {
    ...
    if (_item.type == 'skill') {
      ...
      if (skls != undefined) {
        ui.notifications.warn(`THERE IS ALREADY A SKILL WITH THAT NAME`)
        return false
      }
    }
  }
...
async function createItemMacro(data, slot) {
  if (data.type == 'Item') {
    ui.notifications.notify(
      'Adding Item Macro: This feature is to be added in the future',
    )
    return
  }
