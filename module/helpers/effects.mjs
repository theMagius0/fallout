...
export function outOfAmmo(actorID = null, weapon = null, numDiceRolled = 1, addingRounds = false) {
	...
	if (actorID && weapon) {
		...
		if (actorObj && weaponObj) {
			...
			if (actorObj.data.token.actorLink) {
				let ammoType = weaponObj.data.data.ammo;
				let weaponName = weaponObj.data.name;
				...
				if (addingRounds) {
					...
					additionalAmmo = 1 + (numDiceRolled - weaponObj.data.data.damage.rating);
				}
				...
				if (ammoType) {
					...
					if (ammoObj) {
						ammoBefore = ammoObj.data.data.quantity;
